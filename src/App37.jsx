...
function Child11() {
  ...
  useEffect(() => {
    ...
    // unmount 때 실행되는 코드 :
    // useEffect의 첫번째 파라미터로 들어사는 함수의 리턴되는함수로
    return () => {
...
function Child31() {
  ...
  useEffect(() => {
    console.log("count 업데이트 되면 실행되는 함수의 코드들....");
  }, [count]);
  useEffect(() => {
  ...
  useEffect(() => {
    console.log("count/otherCount 업데이트 되면 실행되는 함수의 코드들...");
  }, [count, otherCount]);

  return (
    <div>
      <button onClick={() => setCount(count + 1)}>자식3 카운트 {count}</button>
...
function Child41() {
  ...
  // 3. count 가 업데아트 될 때 ("count 업데이트 됨") 출력
  useEffect(() => {
    console.log("마운트 됨");
    return () => {
      console.log("언마운트 됨");
